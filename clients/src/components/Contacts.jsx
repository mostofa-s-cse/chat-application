      ...
      <div className='flex flex-col -space-y-1 overflow-y-scroll scrollbar-hide h-[87vh] pb-10'>
        {
          chats?.length > 0 ? chats?.map((e) => {
            return (
              <div onClick={() => {
              ...
              }} key={e.id} className={`flex items-center justify-between sm:gap-x-1 md:gap-x-1 mt-5 ${activeChat.id === e.id ? "bg-[#fafafa]" : "bg-[#fff]"} cursor-pointer  py-4 px-2`}>
                <div className='flex items-center gap-x-3 sm:gap-x-1 md:gap-x-3'>
                  ...
                  <div>
                    ...
                    <p className='text-[13.6px] sm:text-[13.5px] font-medium text-[#56585c] '>{e.latestMessage?.content?.length > 30
                      ? e.latestMessage?.content.slice(0, 30) + "..."
