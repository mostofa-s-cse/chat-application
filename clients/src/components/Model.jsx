...
function Model(props) {
  ...
  const handleClick = async (e) => {
    ...
    await addToGroup({ userId: e?.id, chatId: activeChat?.id })
  ...
  const updateBtn = async () => {
    if (name) {
      let data = await renameGroup({ chatId: activeChat.id, chatName: name })
      if (data) {
        dispatch(fetchChats())
        setOpen(false)
      }
  ...
  const deleteSelected = async (ele) => {
    const res = await removeUser({ chatId: activeChat.id, userId: ele.id })
    if (res.id) {
      setMembers(members.filter((e) => e.id !== ele.id))
  ...
  const leaveGroup = async () => {
    const res = await removeUser({ chatId: activeChat.id, userId: activeUser.id })
    if (res.id) {
      dispatch(fetchChats())
      setOpen(false)
    }
                    ...
                    members.length > 0 && members?.map((e) => {
                      return (
                        <button button className='flex items-center gap-x-1 bg-green-100 text-green-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-green-400 border border-green-400' >
                          <span className='text-[10px]'>{e.id === activeUser.id ? "You" : e.firstName}</span>
          ...
          >
            <Box sx={style}>
              <div className='w-[250px] h-[250px] flex flex-col items-center justify-center -mt-4'>
                ...
                <h3 className='text-[14px] font-semibold text-[#268d61]'>{!activeChat?.isGroup && activeChat?.users[0]?.id === activeUser.id ? activeChat?.users[1]?.email : activeChat?.users[0]?.email}</h3>
                <div className='flex flex-col items-start'>

                  <h5 className='text-[13px]'>{!activeChat?.isGroup && activeChat?.users[0]?.id === activeUser.id ? activeChat?.users[1]?.bio : activeChat?.users[0]?.bio}</h5>
                </div>
              </div>
