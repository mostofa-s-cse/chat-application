import { FaChevronLeft, FaPhone, FaVideo, FaPlus, FaCamera, FaMicrophone, FaPaperPlane, FaDownload, FaReply, FaFile, FaTrash, FaPause, FaPlay, FaEllipsisV } from 'react-icons/fa';
...
const Chat = () => {
    ...
    useEffect(() => {
        ...
        const fetchMessages = async () => {
            try {
                ...
                // For now, we'll use the static messages array
    ...
    const renderMessage = (message: Message) => {
        if (isLoading) {
            ...
            return <SkeletonLoader key={message.id} type="message" variant={message.type === 'outgoing' ? 'outgoing' : 'incoming'} />;
        }
            ...
            case 'incoming':
                return (
                    <div key={message.id} className="flex items-center space-x-2">
                        ...
                        <div className="max-w-[70%] rounded-xl p-3 text-gray-900 leading-tight select-text">
                            {message.images ? (
                                <>
                                        ...
                                        {message.images.map((image, index) => (
                                            <div key={index} className="relative">
                                                <img 
                                                    ...
                                                    className={`w-full ${index === 0 ? 'h-32' : 'h-20'} object-cover ${
                                                        index === 0 ? 'rounded-l-xl rounded-tr-xl' :
                                                        index === 1 ? 'rounded-tr-xl rounded-bl-xl' :
